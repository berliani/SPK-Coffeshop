        ...
        <div class="flex-none w-full max-w-full px-3">
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="flex flex-row items-center justify-between p-6 pb-0 mb-4 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <h6>Tabel {{ $judul }}</h6>
                    <div>
                        <label for="add_button" class="cursor-pointer inline-block px-3 py-2 font-bold text-center text-white rounded-lg text-sm ease-soft-in shadow-soft-md bg-gradient-to-br from-brownPrimary to-brownPrimary/80 shadow-soft-md hover:shadow-soft-xs active:opacity-85 hover:scale-102 transition-all">
                    ...
                    <form action="{{ route('objek.simpan') }}" method="post" enctype="multipart/form-data">
                        <h3 class="font-bold text-lg">Tambah {{ $judul }}</h3>
                            ...
                            <div class="form-control w-full max-w-xs">
                                ...
                                <input type="text" name="alamat" placeholder="Type here" class="input input-bordered w-full max-w-xs text-dark" value="{{ old('alamat') }}" required />
                                <label class="label">
                                    @error('alamat')
                                        <span class="label-text-alt text-error">{{ $message }}</span>
                                    @enderror
                    ...
                    <form action="{{ route('objek.perbarui') }}" method="post" enctype="multipart/form-data">
                        <h3 class="font-bold text-lg">Ubah {{ $judul }}: <span class="text-greenPrimary" id="title_form"><span class="loading loading-dots loading-md"></span></span></h3>
                            ...
                            <div class="form-control w-full max-w-xs">
                                ...
                                <input type="text" name="alamat" placeholder="Type here" class="input input-bordered w-full text-dark" required />
                                <label class="label">
                                    @error('alamat')
                                        <span class="label-text-alt text-error">{{ $message }}</span>
                                    @enderror
        ...
        function edit_button(id) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("input[name='alamat']").val(items[2]); // Mengisi input alamat dengan data yang diambil dari server
